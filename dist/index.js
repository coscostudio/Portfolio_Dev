...
(() => {
  ...
  var VideoCacheManager = class _VideoCacheManager {
    ...
    async preloadVideos(highPriority) {
  ...
  // src/components/video/videoLoader.ts
  function initializeVideo(container) {
  ...
  function setupSliderVideo(video) {
    video.load();
    video.play().catch(() => {
      video.muted = true;
      video.play();
  ...
  function setupStandardVideo(video, options) {
    video.preload = "auto";
  ...
  function setupVideoPlayback(video) {
